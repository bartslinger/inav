...
static void NOINLINE pidApplyFixedWingIndiRateController(pidState_t *pidState, float dT, float dT_inv)
{
    const float rateTarget = getFlightAxisRateOverride(pidState->axis, pidState->rateTarget);
    ...
    axisPID[pidState->axis] = pidState->errorGyroIf;
...
#ifdef USE_BLACKBOX
    axisPID_P[pidState->axis] = (int32_t)(pidState->delayed_command_buffer[command_buffer_tail]);
    axisPID_I[pidState->axis] = (int32_t)(pidState->errorGyroIf);
    axisPID_D[pidState->axis] = 0.0f;
    axisPID_F[pidState->axis] = (int32_t)(pidState->indiModel.state);
    axisPID_Setpoint[pidState->axis] = rateTarget;
#endif
